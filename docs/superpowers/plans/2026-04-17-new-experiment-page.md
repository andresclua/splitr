# New Experiment Page — Progressive Builder Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the "New experiment" slide-over modal with a dedicated page at `/dashboard/[slug]/experiments/new` that shows a progressive builder (expandable nodes) with a mini flow preview.

**Architecture:** Single new Vue SFC (`new.vue`) with inline node blocks; all reactive state lives in `<script setup>`. The existing `POST /api/workspaces/[slug]/experiments` endpoint is used unchanged — only the payload field names must match its schema (`target_url`, `traffic_weight`, `is_control`). The `index.vue` experiments list page loses its modal slide-over and all modal-related script logic; its "New experiment" buttons are replaced with navigation links.

**Tech Stack:** Nuxt 3, Vue 3 `<script setup>` + TypeScript, Tailwind CSS, `$fetch`, `useWorkspace()`, `useToast()`, `navigateTo()`
...
|---|---|---|
| Create | `app/pages/dashboard/[slug]/experiments/new.vue` | Full new experiment page |
| Modify | `app/pages/dashboard/[slug]/index.vue` | Remove modal, update button navigation |

---

## Task 1: Create `new.vue` — Script Setup

**Files:**
- Create: `app/pages/dashboard/[slug]/experiments/new.vue`

- [ ] **Step 1: Create the file with `<script setup>` only (no template yet)**
...
```bash
cd /path/to/splitr/splitr
pnpm --filter app build 2>&1 | grep -E "error|Error" | head -20
```
...
```bash
git add app/pages/dashboard/\[slug\]/experiments/new.vue
git commit -m "feat: add new experiment page script setup"
```

---

## Task 2: Template — Header + Layout + Traffic Node (Node 1)

**Files:**
- Modify: `app/pages/dashboard/[slug]/experiments/new.vue`

Replace the `<template>` stub from Task 1 with the real template, starting with the page wrapper, header, layout skeleton, and the Traffic node only.

- [ ] **Step 1: Replace `<template>` with header + layout + Traffic node**
...
```bash
# In the splitr/splitr directory:
pnpm dev
```

Navigate to `http://localhost:3000/dashboard/<your-slug>/experiments/new`.

Expected:
- Page header shows "← All experiments | New experiment"
- Traffic node is expanded with a URL input
- Below it, a dashed placeholder shows "🔀 Experiment"
- Entering a URL and clicking "Confirm →" collapses the Traffic node with ✓ badge

- [ ] **Step 3: Commit**

```bash
git add app/pages/dashboard/\[slug\]/experiments/new.vue
git commit -m "feat: add new experiment page header, layout, and traffic node"
```

---

## Task 3: Template — Experiment Node (Node 2) + Variants Node (Node 3)

**Files:**
- Modify: `app/pages/dashboard/[slug]/experiments/new.vue`

Add nodes 2 and 3 inside the builder column, after the connector arrow block that follows node 1.

- [ ] **Step 1: Add the Experiment node (node 2) after the node-1 confirmed connector**

In the builder column, after the comment `<!-- nodes 2–4 added in Task 3 & 4 -->`, add:
...
```vue
        ...
        <template v-if="confirmed.traffic">
          ...
          >
            ...
            <label class="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Experiment name</label>
            <input
              v-model="expName"
              type="text"
...
With the dev server running, navigate to `/dashboard/<slug>/experiments/new`.

Expected:
- Confirming Traffic expands the Experiment node
- The type selector works (Edge / SDK toggles)
- Confirming Experiment expands the Variants node
- Variants can be added and removed; weights auto-balance
- The weight counter shows green `100/100` when balanced
- Confirming Variants with missing URLs shows a toast error
- Confirming Variants with correct data collapses it with ✓ and shows the pending node 4 placeholder

- [ ] **Step 3: Commit**

```bash
git add app/pages/dashboard/\[slug\]/experiments/new.vue
git commit -m "feat: add experiment and variants builder nodes"
```

---

## Task 4: Template — Conversion Goal Node (Node 4) + Mini Preview

**Files:**
- Modify: `app/pages/dashboard/[slug]/experiments/new.vue`

Add node 4 (Conversion Goal) in the builder column, and fill the preview column on the right.

- [ ] **Step 1: Add the Conversion Goal node (node 4) after the node-3 confirmed connector**

Replace the comment `<!-- nodes 3–4 placeholder ends; node 4 added in Task 4 -->` with:
...
```vue
        ...
        <template v-if="confirmed.variants">
          ...
          >
            ...
            <label class="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Conversion URL</label>
            <input
...
- [ ] **Step 2: Fill the preview column**

Replace `<!-- preview added in Task 4 -->` with:
...
```vue
        ...
        <div :class="['text-xs font-semibold px-3 py-1.5 rounded-full text-center', conversionUrl.trim() ? 'bg-blue-50 text-blue-600' : 'border border-dashed border-gray-300 text-gray-400']">
          🎯 {{ conversionUrl.trim() ? new URL(conversionUrl).hostname : 'Goal?' }}
        </div>
```

> Note: the `new URL(conversionUrl)` in the preview pill will throw if the URL is invalid. Wrap it in a try/catch by using a computed instead. Add this computed to the script setup:
>
> ```typescript
...
> ```
>
> Then in the template, replace `new URL(conversionUrl).hostname` with `previewConvHost`.

- [ ] **Step 3: Verify the full flow in browser**

Expected behavior end-to-end:
1. Enter a URL in Traffic → Confirm → node collapses with ✓, Experiment node expands
2. Enter experiment name, pick type → Confirm → Variants node expands
3. Fill variant URL, weight shows 100/100 → Confirm → Conversion goal expands
4. Click "Skip & create" → redirects to experiment detail page
5. The mini preview on the right updates live as each step is confirmed
6. Clicking a completed node re-opens it and resets subsequent steps to pending (values preserved)

- [ ] **Step 4: Commit**

```bash
git add app/pages/dashboard/\[slug\]/experiments/new.vue
git commit -m "feat: add conversion goal node and live flow preview"
```

---

## Task 5: Update `index.vue` — Remove Modal, Update Navigation

**Files:**
- Modify: `app/pages/dashboard/[slug]/index.vue`

Remove all modal-related code and replace "New experiment" button targets with navigation to `/new`.

- [ ] **Step 1: Remove modal script logic from `<script setup>`**

In `app/pages/dashboard/[slug]/index.vue`, delete the entire block between `// ── New experiment panel ──────────────────────────────────` and the next major comment block (`// ── Status actions ────────────────────────────────────────`). That block includes:
...
- [ ] **Step 2: Update the "New experiment" header button (line ~186)**
...
- [ ] **Step 3: Update the empty-state "Create experiment →" button (line ~255)**
...
- [ ] **Step 4: Remove the modal slide-over Teleport block**

Delete the entire `<!-- New experiment slide-over -->` block from the template (the `<Teleport to="body">` element and everything inside it, ending at the closing `</Teleport>`). This is lines 400–562 in the current file.

- [ ] **Step 5: Verify in browser**

Expected:
- Clicking "New experiment" on the experiments list page navigates to `/dashboard/<slug>/experiments/new`
- The modal slide-over no longer appears
- The empty-state "Create experiment →" link also navigates to the new page
- The experiments list page still works for viewing, filtering, and status actions

- [ ] **Step 6: Commit**

```bash
git add app/pages/dashboard/\[slug\]/index.vue
git commit -m "feat: replace new experiment modal with dedicated page navigation"
```
...
**Spec coverage:**
- ✅ Dedicated page at `/dashboard/[slug]/experiments/new` — Task 1/2
- ✅ Progressive builder: nodes expand one at a time — Tasks 2/3/4
- ✅ Traffic node: URL input, confirm collapses — Task 2
- ✅ Experiment node: name + type selector — Task 3
- ✅ Variants node: control + N variants, auto-balance, add/remove — Task 3
- ✅ Conversion goal node: optional, skip or create — Task 4
- ✅ Completed nodes clickable to re-open; subsequent nodes reset to pending — Tasks 2/3/4 (`reopenStep`)
- ✅ Values preserved when re-opening earlier nodes — reactive refs, not cleared on `reopenStep`
- ✅ Mini preview column — Task 4
- ✅ On create → redirect to detail page — Task 1 (`navigateTo`)
- ✅ Error toast on API failure — Task 1
- ✅ Remove modal from `index.vue`, update button navigation — Task 5
- ✅ API payload uses correct field names: `target_url`, `traffic_weight`, `is_control` — Task 1

**Type consistency:**
- `VariantDraft` is defined in Task 1 and used unchanged in Tasks 3/4
- `Step` type is defined in Task 1; `reopenStep(step: Step)` is used consistently in Tasks 2/3
- `previewHost` and `previewConvHost` computed are defined in Task 1 (added note in Task 4) — make sure to add `previewConvHost` to the Task 1 script block
